...
/**
 * 인바운드 포트 — 코스 저장(saved_courses)·저장 코스 조회·저장 폴더(saved_course_folders) 공개 API.
 *
...
interface SavedCourseUseCase {
    ...
    /**
     * 저장 폴더를 만든다. 같은 사용자 안에서 이름이 겹치면 중복 생성으로 막는다(409).
     * 새 폴더는 폴더 칩 맨 뒤(order_no = 현재 최대값 + 1)에 붙는다.
     */
    fun createFolder(
        userId: Long,
        name: String,
    ...
    /**
     * 사용자의 저장 폴더를 order_no 순으로 반환한다(폴더 목록 조회 API·저장 시트).
     * 폴더를 고르는 화면은 폴더별 저장 개수가 필요 없어 id·이름만 담는다 — 개수가 필요하면 [getFolderCounts].
     */
    fun getFolders(userId: Long): List<CourseFolderSummary>

    /**
     * 저장함 코스 탭 폴더 칩 재료 — 폴더별 저장 코스 개수(order_no 순) + 폴더 없이 저장한 개수.
     * 저장 목록 화면(BFF)만 쓴다.
     */
    fun getFolderCounts(userId: Long): SavedCourseFolderCounts
}
