...
/**
 * 코스 복제 요청 — 웹 어댑터 DTO. 노션 "코스 복제" 페이지가 본문 미작성이라
 * 기능 정의서(7.2 코스 복제하기 → **7.2.1 게시물 만들기**)와 코스 생성 계약에서 도출했다.
 *
 * 코스 복제가 원본에서 가져오는 것은 **장소 구성(어디를 어떤 순서로)** 뿐이고, 그 위에 올라가는 콘텐츠는
 * 전부 코스 복제하는 사람이 새로 쓴다 — 장소별 caption·사진은 물론 제목·설명·커버·태그·공개 설정까지
 * 코스 만들기와 같은 빌더 화면에서 입력한다. 그래서 요청 모양이 코스 생성([CreateCourseRequest])·
 * 편집([EditCourseRequest])과 같고, 원본 id 는 바디가 아니라 경로(`{courseId}`)로 받는다
 * (실구현 시 courses.duplicated_from_id 로 저장 — 출처 표시용).
 *
 ...
 * - isPublished: true 면 발행(바로 내 게시물), false 면 임시저장 — 코스 복제 후 이어서 편집할 수 있다.
 * - visibility: 공개 범위. 원본 공개범위를 승계하지 않고 코스 복제하는 사람이 정한다.
 * - places 요소는 코스 생성과 동일한 [CreateCoursePlaceRequest] 를 재사용한다 — placeId·orderNo 는
 *   원본에서 프리필되지만 caption·imageUrls 는 사용자가 새로 채운 값이 온다.
 *   **서로 다른 원본 장소를 2곳 이상 담아야 한다**. 순서는 바꿀 수 있고 장소 추가는 자유롭다(최대 10곳).
 */
data class DuplicateCourseRequest(
