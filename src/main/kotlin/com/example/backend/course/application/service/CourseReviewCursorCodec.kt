...
/**
 * 코스 리뷰 목록의 정렬 키(평점·작성일·id)를 URL-safe Base64 불투명 커서로 변환한다
 * ([com.example.backend.place.application.service.PlaceReviewCursorCodec] 와 같은 형식).
 * 클라이언트는 값을 해석하지 않고 그대로 되돌려 준다.
 */
internal object CourseReviewCursorCodec {
    fun encode(cursor: CourseReviewCursor): String {
    ...
    fun decode(cursor: String?): CourseReviewCursor? {
