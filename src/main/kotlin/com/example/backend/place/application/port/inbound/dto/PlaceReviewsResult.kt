...
/**
 * 인바운드 포트 반환 DTO — 장소 리뷰 한 페이지 + 장소 전체 집계.
 * 작성자는 id([PlaceReviewItem.userId])만 담는다 — 닉네임·프로필 이미지 병합은 화면 조합(BFF)의 몫이다.
 *
 * - [averageRating] 살아있는 리뷰의 평점 평균(소수 첫째 자리 반올림). 리뷰가 없으면 0.0.
 * - [totalCount] 리뷰 총 개수(페이지가 아니라 장소 전체).
 * - [ratingDistribution] 5~1점 각각의 개수(없는 별점도 count=0 으로 항상 채운다).
 * - [photoCount] 이 장소 리뷰에 달린 사진 총 개수.
...
data class PlaceReviewsResult(
    ...
    val hasNext: Boolean,
    val reviews: List<PlaceReviewItem>,
) {
    /** 별점별 리뷰 개수. */
     ...
     * 도메인 enum(PlaceReviewTag)은 경계 밖으로 내보내지 않고 표시 값만 옮긴다.
     */
    data class Tag(
        val code: String,
        val label: String,
