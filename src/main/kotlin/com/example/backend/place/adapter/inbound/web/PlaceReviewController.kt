...
/**
 * 인바운드 어댑터 — 장소 리뷰 작성·삭제(노션 명세 · Place · place-review). **모킹 API**.
 * 노션 명세서의 "장소 리뷰 생성" 페이지 본문이 비어 있어
 * 필드는 디자인에서 도출했다(밴드 I · 장소 리뷰 작성).
 *
 * **목록 조회는 이 컨트롤러에 없다** — 후기 목록 화면이 작성자 프로필(user)까지 함께 그리는 화면 조합이라
 * BFF([com.example.backend.mobile.place.adapter.inbound.web.PlaceReviewScreenController],
 * `GET /service/v1/places/{placeId}/reviews`)가 담당한다.
 *
