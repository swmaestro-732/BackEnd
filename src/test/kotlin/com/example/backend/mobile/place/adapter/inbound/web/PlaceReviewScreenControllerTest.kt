...
/**
 * 장소 후기 전체보기 BFF(`GET /service/v1/places/{placeId}/reviews`) 통합 테스트.
 * 리뷰·사진·태그·작성자를 심어 두고 정렬(sort×order)·커서 페이지·집계·작성자 병합(탈퇴자 대체 닉네임)을 HTTP 로 확인한다.
 * 픽스처(place-review-fixture)는 장소 601 하나만 두므로 리뷰는 테스트가 직접 심는다 — 1인 1리뷰라 리뷰마다 작성자가 다르다.
 */
@AutoConfigureMockMvc
@Sql(scripts = ["/sql/place-review-fixture.sql"], executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
class PlaceReviewScreenControllerTest
    @Autowired
    constructor(
        ...
        fun `기본은 최신순이고 집계·사진·태그·작성자가 함께 내려간다`() {
            ...
            mockMvc
                .perform(get("/service/v1/places/$PLACE_ID/reviews"))
                ...
                .andExpect(jsonPath("$.data.hasVisitedPlace").value(false))
                ...
                .andExpect(jsonPath("$.data.reviews[2].tags[0].code").value("coffee"))
                .andExpect(jsonPath("$.data.reviews[2].tags[1].code").value("view"))
        }
        ...
        @Test
        fun `로그인 조회자도 방문 여부는 스텁 false 다`() {
            seedReviews()

            mockMvc
                .perform(
                    get("/service/v1/places/$PLACE_ID/reviews")
        ...
        fun `잘못된 커서는 400 이다`() {
            mockMvc
                .perform(get("/service/v1/places/$PLACE_ID/reviews?cursor=not-a-cursor"))
        ...
        fun `mock=true 면 DB 와 무관하게 고정 목을 내려준다`() {
            mockMvc
                .perform(get("/service/v1/places/999999/reviews?mock=true"))
                .andExpect(status().isOk)
        ...
        /**
         * 장소 601 에 리뷰 3건(+소프트 삭제 1건). 작성일·평점을 서로 다르게 두어 정렬이 결정적이다.
        ...
        private fun seedReviews() {
            ...
            jdbcTemplate.update(
                """
                INSERT INTO place_reviews (id, place_id, user_id, rating, content, status, created_at, updated_at, deleted_at) VALUES
                  (1, $PLACE_ID, $AUTHOR_A, 5, '최고예요', 'PUBLISHED', '2026-01-01T10:00:00Z', '2026-01-01T10:00:00Z', NULL),
                  (2, $PLACE_ID, $AUTHOR_B, 3, NULL,      'PUBLISHED', '2026-01-02T10:00:00Z', '2026-01-02T10:00:00Z', NULL),
                  (3, $PLACE_ID, $MISSING_AUTHOR, 5, '뷰 좋음', 'PUBLISHED', '2026-01-03T10:00:00Z', '2026-01-03T10:00:00Z', NULL),
                  (4, $PLACE_ID, 9999, 1, '삭제됨', 'DELETED', '2026-01-04T10:00:00Z', '2026-01-04T10:00:00Z', '2026-01-05T10:00:00Z')
                """.trimIndent(),
            )
            jdbcTemplate.update(
                "INSERT INTO place_review_photos (place_review_id, image_url, order_no) VALUES (1, 'https://cdn/p1.jpg', 0), (1, 'https://cdn/p2.jpg', 1)",
            )
            jdbcTemplate.update(
                "INSERT INTO place_review_tag_links (place_review_id, tag) VALUES (1, 'COFFEE'), (1, 'VIEW')",
            )
            // totalCount·averageRating 은 행 집계가 아니라 비정규화 카운터에서 읽는다 — 쓰기 경로가 유지하는 값을 그대로 맞춰 둔다.
            // 5+3+5 / 3건(삭제 제외)
            jdbcTemplate.update("UPDATE places SET rating_sum = 13, rating_cnt = 3 WHERE id = $PLACE_ID")
        }

        private fun fetch(query: String): Map<String, Any?> {
            val body =
                mockMvc
                    .perform(get("/service/v1/places/$PLACE_ID/reviews?$query"))
                    .andExpect(status().isOk)
                    .andReturn()
                    .response
                    .contentAsString
