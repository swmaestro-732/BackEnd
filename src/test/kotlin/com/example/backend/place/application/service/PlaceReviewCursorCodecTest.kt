...
class PlaceReviewCursorCodecTest {
    ...
    private fun decode(cursor: String?): PlaceReviewCursor? =
        PlaceReviewCursorCodec.decode(PlaceReviewSortKey.RATING, true, cursor)
    ...
    fun `encode-decode 라운드트립은 마이크로초 정밀도까지 보존한다`() {
        ...
        val decoded = decode(PlaceReviewCursorCodec.encode(PlaceReviewSortKey.RATING, true, cursor))
    ...
    fun `정렬 기준이나 방향이 요청과 다르면 잘못된 커서다`() {
        val encoded =
            PlaceReviewCursorCodec.encode(
                PlaceReviewSortKey.RATING,
                true,
                PlaceReviewCursor(rating = 4, createdAt = Instant.parse("2026-08-01T00:00:00Z"), id = 7),
            )

        assertThatThrownBy { PlaceReviewCursorCodec.decode(PlaceReviewSortKey.LATEST, true, encoded) }
            .isInstanceOf(BusinessException::class.java)
            .extracting("errorCode")
            .isEqualTo(CommonErrorCode.INVALID_INPUT)
        assertThatThrownBy { PlaceReviewCursorCodec.decode(PlaceReviewSortKey.RATING, false, encoded) }
            .isInstanceOf(BusinessException::class.java)
            .extracting("errorCode")
            .isEqualTo(CommonErrorCode.INVALID_INPUT)
    }
